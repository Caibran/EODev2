...
void Map::WalkPlayer(int ID, int direction, int DestX, int DestY)
{
	...
	if (m_PlayerID >= 0)
	{
		...
		if (m_Players.count(m_PlayerID) > 0)
		{
			...
			if (FromX < 0) FromX = 0;
			if (FromY < 0) FromY = 0;
			if (FromX >= this->m_emf.header.width) FromX = this->m_emf.header.width - 1;
			if (FromY >= this->m_emf.header.height) FromY = this->m_emf.header.height - 1;
...
constexpr float epi = 0.00001f; // gap between each interleaved layer
	...
	int  startsuboffset = this->SafeLUTLookup(PlayerMinX, PlayerMinY);
	int  endsuboffset = this->SafeLUTLookup(PlayerMaxX, PlayerMaxY);
	...
	for (std::map<int, Map_Player*>::iterator player = this->m_Players.begin(); player != m_Players.end(); ++player)
	{
		...
		{
			depth = layer_info[layer].depth;
			depth -= (this->SafeLUTLookup(player->second->x, player->second->y) * ep);
			if (rendering)
			{
	...
	{
		depth = layer_info[layer].depth;
		depth -= (this->SafeLUTLookup(NPC->second->x, NPC->second->y) * ep);
		if (rendering)
		{
			next_depth();
			int xoffsp = layer_info[layer].xoff - xoff;
	...
	{
		depth = layer_info[layer].depth;
		depth -= (this->SafeLUTLookup(m_item->second.x, m_item->second.y) * ep);
